      ...
      <form @submit.prevent="add" @keydown="form.onKeydown($event)">
        ...
        <div class="form-group row">
          ...
          <div class="col-md-7">
            ...
            <a href="#" class="btn btn-outline-primary btn-sm" @click.prevent="saveLocation">Save Location</a>
          </div>
        </div>
...
export default {
  ...
  watch: {
    company: function () {
        ...
        this.certifications = this.company.certifications.map(function (certification) { 
          return {name: certification.name, type: certification.pivot.type }
        })
        this.services = this.company.services.map(function (service) { return service.name })
        this.products = this.company.products.map(function (product) { return product.name })
    }
  },
  ...
  methods: {
    async add () {
      ...
      const { data } = await this.form.patch('/api/company/' + this.$route.params.id)
    },
    ...
    saveLocation () {
      ...
      if (this.editing_location_index != null) {
        ...
        if (address_value !== this.editing_location.address) {
          loc_info['lat'] = (this.place) ? this.place.geometry.location.lat() : ''
          loc_info['lng'] = (this.place) ? this.place.geometry.location.lng() : ''
        } else {
